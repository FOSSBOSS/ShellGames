...
This is becuase it uses whatever font, instead of raw ascci.
...
class AsciiArtApp:
    def __init__(self, width=80, height=80):
        self.width = width
        self.height = height
        self.char_grid = [[chr(1)] * width for _ in range(height)]
        self.canvas = None
    ...
    def render_text(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                color = self.text_color if self.char_grid[y][x] != chr(1) else "red"
                self.canvas.create_text(x * 8, y * 14, text=self.char_grid[y][x], anchor="nw", fill=color, font=self.font)
    ...
    def on_drag(self, event):
        ...
        #self.pressure = random.choice([0, 1, 2, 3])  
        self.pressure = random.choice([0, 1])  
        #char_map = {0: chr(0), 1: chr(11), 2: chr(22), 3: chr(33)}
        char_map = {0: chr(55), 1: chr(111)}
    ...
    def reset_screen(self, event=None):
        ...
        global i
        self.char_grid = [[chr(i)] * self.width for _ in range(self.height)]
        i=i+1
        print(i)
        self.render_text()
